# Legal And Business Checklist

This is not legal or tax advice. It records the unresolved decisions that block the first paid contract and the preliminary checks already completed.

## Blocking Decisions

- Register the legal operator in Slovakia before accepting paid client work.
- Confirm the operator's full legal name, legal form, official business address, and registration number.
- Confirm VAT status, invoice wording, record-retention duties, and which tax authority receives filings.
- Confirm governing law, competent courts, professional liability position, and the client agreement wording.
- Keep the Slovak data-protection authority named in the privacy policy.
- Ask a Slovak trade-mark professional to confirm the Locriva similarity search before filing a mark or making substantial brand investment.

## Business Setup

- Choose the correct Slovak legal form and obtain the required trade authorisation.
- Confirm the registered address, tax registration, and cross-border VAT treatment with a Slovak accountant or business-registration adviser.
- Set up a way to invoice clients.
- Set up a business bank/payment method.
- Track income and expenses from day one.
- Understand tax obligations in your country/state.
- Add the official legal identity, legal form, register, registration number, VAT ID if issued, service address, and business contact details to the Legal Notice before formal paid contracting.

## Website Compliance

- Use real contact/business information where required.
- Do not use fake reviews or fake testimonials.
- Get permission before using client logos, reviews, or photos.
- Use a privacy policy when collecting form submissions or analytics data.
- If serving EU customers, check GDPR, cookie consent, service-provider information, invoice rules, and consumer rights if you sell to consumers.
- Keep the current site free of tracking cookies unless the cookie policy and consent flow are updated.
- Do not guarantee SEO rankings or business results.

## Client Protection

- Use written scope before starting.
- Take a deposit.
- Define revision rounds.
- Define who owns domain and hosting accounts.
- Define maintenance responsibilities.
- Keep a launch checklist.

## Inputs Needed From You

- Slovakia is the planned country of establishment.
- Full legal operator name, legal form, registered address, register, and registration number.
- VAT status and VAT ID if issued.
- Governing law and dispute venue confirmed by professional advice.
- Preferred business name: Locriva, subject to final professional clearance.
- Your contact email: daniellaky.uni@gmail.com.
- Payment method you can accept.
- Whether you want to sell in English, Slovak, Dutch, German, or another language.

## Preliminary Name And Domain Record

- Checked on 24 July 2026.
- EUIPO eSearch exact result for Locriva: none.
- TMview exact result for Locriva: none across participating EU, national, and international offices.
- TMview fuzzy results: similar names existed in unrelated goods and services; no result identified in website design or software services during this preliminary review.
- Slovak Commercial Register exact result for Locriva: none.
- Verisign RDAP status for locriva.com: not found, indicating unregistered at the time checked.
- SK-NIC WHOIS status for locriva.sk: domain not found at the time checked.
- Domain and filing availability can change at any time. Recheck immediately before registration.

## Official Starting Points

- Slovakia government business portal: https://www.slovensko.sk/en/
- Slovak Commercial Register: https://www.orsr.sk/
- Slovak Industrial Property Office: https://www.indprop.gov.sk/en/
- EUIPO eSearch: https://euipo.europa.eu/eSearch/
- TMview: https://www.tmdn.org/tmview/
- SK-NIC WHOIS: https://whois.sk-nic.sk/?lang=en
- Slovak data-protection authority: https://dataprotection.gov.sk/
- EU business website requirements: https://europa.eu/youreurope/business/running-business/digitalising/setting-up-business-website/index_en.htm
